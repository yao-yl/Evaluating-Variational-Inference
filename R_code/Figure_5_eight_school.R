...
est_k_hat=function(vi_marg, marginal_samples){
  ...
  approx_target <- approx(true_density$x, log(true_density$y), vi_marg,rule = 1,yleft=-1e10, yright=-1e10)
  log_target_coordinate<-approx_target$y
  ip_ratio_coordinate<-as.vector(log_target_coordinate-log_proposal_coordinate)
  marginal_k_hat_true<-psis(log_ratios=ip_ratio_coordinate)$diagnostics$pareto_k
...
stan_samples=extract(fit_school2)
mu_sample=stan_samples[["mu"]]
tau_sample=stan_samples[["tau"]]

#fit_vb_temp=fit_vb
m=stan_model(model_code = school_code)
fit_vb=vb( m,  data=list(y=y, sigma=sigma, J=8), iter=1e6,output_samples=1e5,tol_rel_obj=0.001)
...
fit_vb2=vb( m2,  data=list(y=y, sigma=sigma, J=8), iter=3e6,output_samples=1e5,tol_rel_obj=0.01)
vb_sample2=extract(fit_vb2)
...
joint_diagnoistics=psis(log_ratios=ip_ratio)
joint_diagnoistics$diagnostics$pareto_k
