...
#   fit_vb=vb( m, data=list(x=x,y=y_sim, D=D,N=N),iter=5e4,output_samples=1e4,tol_rel_obj=0.01,eta=1,adapt_engaged=F)
...
{
  mu_0 = rnorm(1,0, 50)
  tau_0 = rhalfcauchy(1, scale=25)
  theta_0= rnorm(8,mu_0, tau_0)
  y_sim = rnorm(8,theta_0, sigma)
  b_0=c(theta_0, mu_0, log(tau_0))
  fit_vb=vb( m,  data=list(y=y_sim, sigma=sigma, J=8), iter=1e5,output_samples=1e4,tol_rel_obj=0.01,eta=1,adapt_engaged=F)
  vb_samples=extract(fit_vb)
...
save(prob, file="~/Desktop/prob.RData")
...
save(prob, prob_rp, file="~/Desktop/school_cgr_cauchy.RData")
load(file="~/Desktop/school_cgr.RData")
...
pdf("~/Desktop/eight_school_cgr_normal.pdf",height=1.4,width=4)
par(mfrow=c(1,3), oma=c(1 ,0.5,0.7,0 ), pty='m',mar=c(0.4,0.4,0,0.5) ,mgp=c(1.5,0.25,0), lwd=0.5,tck=-0.01, cex.axis=0.6, cex.lab=0.8, cex.main=0.9)
 hist(  prob[,1]  ,breaks = 20,axes=F,xlim=c(0,1),xlab="", ylab="",main = "",probability = T,ylim=c(0,10))#,border = c("grey",rep(1,18),"grey" )
  ...
  abline(h=1,col='grey', lty=2)
  hist(prob_rp[,10],breaks = 20,axes=F,xlim=c(0,1),xlab="", ylab="",main = "",probability = T,ylim=c(0,10))
